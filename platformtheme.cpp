...
void PlatformTheme::updateTheme() {
    pal = new QPalette();
   ...
   fixedFnt = new QFont(settings->value("fonts/monospaceFamily", "Hack").toString(), settings->value("fonts/monospaceSize", 9).toInt());
   smallFnt = new QFont(settings->value("fonts/smallFamily", defaultFont).toString(), settings->value("fonts/smallSize", 8).toInt());
   fnt = new QFont(settings->value("fonts/defaultFamily", defaultFont).toString(), settings->value("fonts/defaultSize", 10).toInt());

   if (settings->value("accessibility/largeText", false).toBool()) {
       fnt->setPointSizeF(fnt->pointSizeF() * 1.25);
       smallFnt->setPointSizeF(smallFnt->pointSizeF() * 1.25);
       fixedFnt->setPointSizeF(fixedFnt->pointSizeF() * 1.25);
   }
}
